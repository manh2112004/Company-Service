...
public class CompanySocialCommandController {
    ...
    @DeleteMapping("/{companyId}/socials/{socialId}")
    public CompletableFuture<String> deleteSocial(
            @AuthenticationPrincipal Jwt jwt,
            @PathVariable String companyId,
            @PathVariable String socialId
    ) {
        return companySocialService.deleteSocial(jwt.getSubject(), companyId, socialId);
    }
}
